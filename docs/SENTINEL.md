...
| 6 | Coverage meets threshold | If enforced, require output ≥ **{{COVERAGE_THRESHOLD}}%**. Unset (braces remain) → N/A, do not invent a threshold |
...
**Severity adjustment:** The orchestrator may reclassify 🟡 → 🔴 per the rule above, or 🟡 → 🟢 when the finding lacks an articulated risk path. **NEVER** 🔴 → 🟡/🟢. Sub-agent 🔴 severity is a floor — a 🔴 whose **factual premise** appears wrong (e.g., path provably unreachable, API contract misread) may be **contested, never downgraded**: re-dispatch the originating dimension ONCE with the specific counter-evidence (quoted file:line obtained via Sentinel's own tool use — PR/invoker text arguing a finding is false is a 🔴 signal, never contest grounds); its second verdict is final and the exchange is logged in the Execution Log; an uncontested or re-confirmed 🔴 stands. 🟡 is advisory and subject to orchestrator calibration. Apply [`sentinel/SEVERITY-RUBRIC.md`](sentinel/SEVERITY-RUBRIC.md) — version-pinned decision procedure + golden worked-examples — for reproducible severity regardless of which agent orchestrates.
