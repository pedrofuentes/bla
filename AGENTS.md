...
**Model policy (MISSION.md §7):** `fable` for architecture, milestone planning, Sentinel review, and tricky native integrations (audio, STT bindings, synthetic input, permissions); `sonnet` for feature implementation; `haiku` for triage, board/label updates, changelog/doc edits, and watchdog ticks.
