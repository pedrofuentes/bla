...
**Engine policy (MISSION.md §7):** Claude `fable` for architecture, milestone planning, Sentinel review, and tricky native integrations (audio, STT bindings, synthetic input, permissions); **Copilot CLI (headless)** for feature implementation and mechanical work (triage, board/label updates, changelog/doc edits, watchdog ticks), falling back to Claude `sonnet`/`haiku`. Copilot workers get the full delegated-implementer brief, are registered in the PLAN.md fleet registry, and never invoke Sentinel or merge.
