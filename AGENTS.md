...
**bla** — local-first, system-wide voice dictation: hold a hotkey, speak, and cleaned, polished text lands at the cursor of the focused app or is appended to a Markdown file — all speech-to-text and AI cleanup runs on-device.

- **Tech stack**: Rust (Tauri 2 core: cpal, whisper-rs, enigo, rusqlite, active-win-pos-rs), TypeScript (React + Vite + Tailwind UI) — versions: Rust stable, Node 20+, Tauri 2.x
- **Package manager**: cargo (core) + pnpm (UI) | **Module system**: ES modules (UI)

## Commands

```bash
cargo test <module>                # Rust core, module-scoped (prefer)
cargo test && cargo clippy -- -D warnings && cargo fmt --check    # full Rust gate
pnpm test -- <path>                # UI unit tests, file-scoped (prefer)
pnpm install | build | test | lint | typecheck | format           # full UI suite
pnpm tauri dev | build             # run / package the app
```
...
2. `cargo test` + `pnpm test` — full suite green on final HEAD
3. `cargo clippy -- -D warnings && cargo fmt --check` + `pnpm lint` — zero warnings
...
**OS-integration exemption:** thin glue that only calls platform APIs (audio-device open, synthetic keystrokes, tray/window management, permission prompts) is treated like scaffolding — TDD-exempt but Sentinel-reviewed; keep it in the designated modules (`audio`, `output`, `hotkeys`, `context`) and keep all logic out of it so the logic stays testable.
...
**Engine policy (MISSION.md §7):** Claude `fable` for architecture, milestone planning, Sentinel review, and tricky native integrations (audio, STT bindings, synthetic input, permissions); **Copilot CLI (headless)** for feature implementation and mechanical work (triage, board/label updates, changelog/doc edits, watchdog ticks), falling back to Claude `sonnet`/`haiku`. Copilot workers get the full delegated-implementer brief, are registered in the PLAN.md fleet registry, and never invoke Sentinel or merge. **Containment precondition (MISSION.md §7):** Copilot implementer spawns (file edits, commits, pushes) are gated until Sentinel-in-CI + the harness-guard are required checks on `main` (or the worker's credential structurally cannot push/merge); read-only Copilot work is permitted now.
...
- **Formatter**: rustfmt (Rust) + Prettier (TS) — run before commit. **Linter**: clippy with `-D warnings` (Rust) + ESLint typescript-recommended (TS) — fix all warnings.
- Conventions: logic behind traits (`Cleanup`, output router) with OS calls confined to the designated modules (`audio`, `output`, `hotkeys`, `context`); named exports and functional components in the UI; LLM prompts live in versioned prompt files with fixture-based regression checks, rewrite-only (never answer, never add content).
...
- Run `cargo test && pnpm test` + linters before PR; invoke Sentinel before merge
...
Auth/crypto/PII · DB migrations · AGENTS.md/SENTINEL.md changes · production deploys (GitHub Releases) · 🔴 CRITICAL findings · 5× Sentinel rejections · deployment pipeline setup · credentials rotation · the per-milestone cofounder smoke test (MISSION.md AC-7)
...
- **Project (MISSION.md §7)**: naming, referencing, or linking any competing dictation product or company in any artifact — describe functionality generically · any product network call outside the MISSION.md §5 allowlist · telemetry/analytics · sending audio or text off-device · logging or persisting raw clipboard contents · committing model files or user data (history DB, settings)
...
| Document | Read when... |
|----------|-------------|
| [`MISSION.md`](./MISSION.md) | Always — the binding project brief |
